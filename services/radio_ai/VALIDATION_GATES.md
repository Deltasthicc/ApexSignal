...
| 6c. xsmall within 3pp of base AND service is CPU-only | Switch to xsmall via `USE_CLASSIFIER_FALLBACK=true` | base best macro-F1 0.258 (threshold 0.45) vs. **xsmall best 0.393 (threshold 0.15) — xsmall now outperforms base by 13.5pp.** | **REOPENED — see correction below, not decided** |
...
- **Gate 6c's conclusion flips**: under the corrected methodology, `xsmall` (0.393) clearly beats `base` (0.258) — the opposite of "keep base regardless of CPU/GPU," which was explicitly locked in during the previous session based on the since-disproven numbers. **I have not switched `USE_CLASSIFIER_FALLBACK` — this reverses an explicit prior decision and needs a real sign-off, not a silent auto-correct.** If you want to revisit: xsmall's per-class F1 at its best threshold (0.15) was `FRONT_TURNIN_BRAKE` 0.800, `VISIBILITY_TRACK_CONDITION` 0.667, `NO_COMPLAINT` 0.780, `MECHANICAL_OTHER` 0.111, `EXIT_TRACTION_REAR`/`TYRE_GRIP_DEGRADATION` 0.000 — a genuinely different error profile from base's, not just a better number.
